window.TIL_FILES = window.TIL_FILES || {};
window.TIL_FILES["data/2026/07/2026-07-01.js"] = [
  {
    "id": "20260701-001",
    "date": "2026-07-01",
    "title": "HUD와 함수 개념",
    "project": "Basecamp",
    "phase": "처음 접함",
    "tags": [
      "C",
      "Unreal",
      "Blueprint",
      "함수",
      "실습"
    ],
    "study_content": "Unreal에서 HUD를 만들고 적 감지, 추적, NavMesh, 자폭 등의 기능을 학습했다. 동시에 C 언어의 함수 개념을 학습했다.",
    "learning_process": "같은 작업을 여러 번 작성하지 않고 한 번 만들어 재사용할 수 있다는 함수의 기본 개념을 접했다.",
    "questions": [
      "함수의 매개변수와 반환값은 어떤 역할을 하는가?"
    ],
    "mistakes_or_difficulties": [
      "함수의 재사용 개념은 이해했지만 매개변수와 반환값은 당시 정확히 이해하지 못했다."
    ],
    "application": "게임 기능을 함수 단위로 구현하는 예제를 따라갔다.",
    "result": "함수를 반복 작업을 재사용하기 위한 코드 단위로 처음 이해하기 시작했다.",
    "code_reference": null,
    "understanding": "핵심 흐름 이해",
    "related_topics": [
      "HUD",
      "NavMesh",
      "함수",
      "AI"
    ],
    "later_connection": "이후 TextRPG에서 여러 기능을 함수로 분리하면서 함수의 필요성을 직접 경험했다.",
    "next_learning": "아이템, 탄약, UI 이벤트 등 게임 기능 확장",
    "velog": "https://velog.io/@jwh4410/7.1",
    "repository": null,
    "source": [
      "Velog"
    ],
    "primary_topic": "unreal",
    "summary": "Unreal에서 HUD를 만들고 적 감지, 추적, NavMesh, 자폭 등의 기능을 학습했다.",
    "date_start": "2026-07-01",
    "date_label": "2026-07-01"
  }
];
